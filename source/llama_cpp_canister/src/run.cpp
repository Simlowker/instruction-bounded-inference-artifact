...
void run(IC_API &ic_api, const uint64_t &max_tokens, bool is_query) {
  ...
  // ICPP-PATCH: Model lifecycle is now managed by static vars in main_().
  // Don't free on every call — the static model persists across calls to avoid
  // WASM fragmentation/OOM. icpp_free_model() is only for explicit model swap.
