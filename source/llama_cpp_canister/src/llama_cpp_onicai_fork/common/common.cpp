...
#ifdef __wasi__
    return "2026_01_01-00_00_00.000000000"; // WASI/ICP: no clock
#else
...
#ifdef __wasi__
    // WASI/ICP: std::filesystem crashes — use stat() fallback
    struct stat st;
...
#if defined(__wasi__)
    // WASI/ICP: no filesystem cache, getenv() may crash
...
#ifdef __wasi__
    // WASI/ICP: std::filesystem::directory_iterator crashes — use opendir/readdir fallback
...
#ifdef __wasi__
    // WASI/ICP: no TTY, no colors, and getenv() may crash
    return false;
#else
...
#ifdef __wasi__
    // WASI/ICP: disable warmup — it triggers exceptions that cannot be caught
    // (no C++ exception runtime on WASI). The warmup is just a performance
    // optimization that pre-fills caches; skipping it is safe.
    params.warmup = false;
#endif
    if (params.warmup) {
