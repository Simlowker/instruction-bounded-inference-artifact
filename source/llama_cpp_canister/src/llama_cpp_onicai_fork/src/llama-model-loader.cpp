...
bool llama_model_loader::load_all_data(
    ...
    LLAMA_LOG_INFO("%s: tensor load start: tensors=%d total_data=%.2f MiB mmap=%s check=%s\n",
            __func__, n_tensors, size_data / 1024.0 / 1024.0,
            use_mmap ? "true" : "false",
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        const bool is_large_tensor = n_size >= 64 * MiB;
        const bool is_last_tensor = size_done + n_size >= size_data;
        const bool should_log_progress =
                tensor_loaded == 1 ||
                tensor_loaded % 16 == 0 ||
                is_large_tensor ||
                is_last_tensor;
        ...
        if (should_log_progress) {
            LLAMA_LOG_INFO(
                    "%s: tensor %zu/%d name=%s type=%s size=%.2f MiB file=%u offs=%.2f MiB cum_before=%.1f%%\n",
                    __func__,
                    tensor_loaded,
                    n_tensors,
                    ggml_get_name(cur),
                    ggml_type_name(cur->type),
                    n_size / 1024.0 / 1024.0,
                    (unsigned) weight->idx,
                    weight->offs / 1024.0 / 1024.0,
                    size_data ? (100.0 * size_done / size_data) : 0.0);
        }
        ...
        if (use_mmap) {
            ...
            if (check_tensors) {
#ifdef __wasi__
                // WASI/ICP: no threads — validate synchronously
                validation_result.emplace_back(std::async(std::launch::deferred, [cur, data, n_size] {
...
#ifdef __wasi__
                    // WASI/ICP: no threads — validate synchronously
                    validation_result.emplace_back(std::async(std::launch::deferred, [cur, n_size] {
