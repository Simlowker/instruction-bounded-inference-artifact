...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    for (auto & [buft, ctx_ptr] : ml.ctx_map) {
        ...
        const size_t planned_buffer_size = ml.no_alloc
                ? 0
                : ggml_backend_alloc_ctx_tensors_from_buft_size(ctx, buft);

        LLAMA_LOG_INFO(
                "%s: preparing backend buffer type=%s device=%s tensors=%zu planned=%.2f MiB mmap=%s host_ptr=%s default=%s\n",
                __func__,
                ggml_backend_buft_name(buft),
                ggml_backend_dev_name(dev),
                ctx_tensor_count,
                planned_buffer_size / 1024.0 / 1024.0,
                ml.use_mmap ? "true" : "false",
                buffer_from_host_ptr_supported ? "true" : "false",
                is_default_buft ? "true" : "false");
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
                ...
                LLAMA_LOG_INFO(
                        "%s: buffer_from_host_ptr file=%u range=[%.2f, %.2f) MiB span=%.2f MiB max_tensor=%.2f MiB\n",
                        __func__,
                        idx,
                        first / 1024.0 / 1024.0,
                        last / 1024.0 / 1024.0,
                ...
                LLAMA_LOG_INFO(
                        "%s: mapped backend buffer type=%s file=%u actual=%.2f MiB\n",
                        __func__,
                        ggml_backend_buft_name(buft),
                        idx,
            ...
            LLAMA_LOG_INFO(
                    "%s: allocated backend buffer type=%s actual=%.2f MiB host=%s\n",
                    __func__,
                    ggml_backend_buffer_name(buf),
                    ggml_backend_buffer_get_size(buf) / 1024.0 / 1024.0,
                    ggml_backend_buffer_is_host(buf) ? "true" : "false");
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_LLAMA4:
            {
                // llama-iswa.cpp not compiled for ICP — abort
                GGML_ABORT("LLAMA4 model not compiled for ICP");
            } break;
        case LLM_ARCH_LLAMA_EMBED:
            {
                llm = std::make_unique<llm_build_llama<true>>(*this, params);
            } break;
#if 0  // Models not compiled for ICP — reduce WASM globals
        case LLM_ARCH_MAINCODER:
            {
...
#endif  // end Models not compiled for ICP
        case LLM_ARCH_QWEN2:
            {
                llm = std::make_unique<llm_build_qwen2>(*this, params);
            } break;
#if 0  // More models not compiled for ICP
        case LLM_ARCH_DREAM:
            {
...
#endif  // end More models not compiled for ICP
        // ICP-enabled additional architectures
        case LLM_ARCH_QWEN3:
            {
                llm = std::make_unique<llm_build_qwen3>(*this, params);
            } break;
        ...
        // LFM2: requires hybrid memory — disabled for now
        // case LLM_ARCH_LFM2:
        case LLM_ARCH_GPT2:
            {
                llm = std::make_unique<llm_build_gpt2>(*this, params);
        ...
        default:
            GGML_ABORT("unsupported model architecture for ICP: %s", llm_arch_name(arch));
    }
