...
// ICPP-PROFILING: ICP instruction counter for deep decode profiling
#ifdef __wasm__
__attribute__((import_module("ic0")))
__attribute__((import_name("performance_counter")))
extern "C" uint64_t icpp_perf_counter_ctx(uint32_t counter_type);
#else
static inline uint64_t icpp_perf_counter_ctx(uint32_t t) { (void)t; return 0; }
#endif
static uint64_t icpp_prof_graph_build = 0;
static uint64_t icpp_prof_graph_alloc = 0;
static uint64_t icpp_prof_set_inputs = 0;
static uint64_t icpp_prof_graph_compute = 0;
static int icpp_prof_decode_calls = 0;
...
llama_context::llama_context(
    ...
    {
#ifdef __wasi__
        // WASI/ICP: getenv() crashes — disable graph reuse check
        (void)graph_reuse_disable;
#else
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    uint64_t _p0 = icpp_perf_counter_ctx(0);
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        uint64_t _p1 = icpp_perf_counter_ctx(0);
        icpp_prof_graph_build += _p1 - _p0;
        ...
        uint64_t _p2 = icpp_perf_counter_ctx(0);
        icpp_prof_graph_alloc += _p2 - _p1;
    }

    // set the input data for the input tensors
    {
        uint64_t _p3 = icpp_perf_counter_ctx(0);
        res->set_inputs(&ubatch);
        icpp_prof_set_inputs += icpp_perf_counter_ctx(0) - _p3;
    }

    uint64_t _p4 = icpp_perf_counter_ctx(0);
    const auto status = graph_compute(res->get_gf(), ubatch.n_tokens > 1);
    icpp_prof_graph_compute += icpp_perf_counter_ctx(0) - _p4;
    icpp_prof_decode_calls++;
...
// ICPP-PROFILING: dump decode breakdown
void icpp_dump_decode_profile() {
    uint64_t total = icpp_prof_graph_build + icpp_prof_graph_alloc + icpp_prof_set_inputs + icpp_prof_graph_compute;
    fprintf(stderr, "ICPP-PROF: === DECODE BREAKDOWN (%d calls) ===\n", icpp_prof_decode_calls);
    fprintf(stderr, "ICPP-PROF: graph_build=%llu (%.1f%%)\n", icpp_prof_graph_build, total ? 100.0*icpp_prof_graph_build/total : 0);
    fprintf(stderr, "ICPP-PROF: graph_alloc=%llu (%.1f%%)\n", icpp_prof_graph_alloc, total ? 100.0*icpp_prof_graph_alloc/total : 0);
    fprintf(stderr, "ICPP-PROF: set_inputs=%llu (%.1f%%)\n", icpp_prof_set_inputs, total ? 100.0*icpp_prof_set_inputs/total : 0);
    fprintf(stderr, "ICPP-PROF: graph_compute=%llu (%.1f%%)\n", icpp_prof_graph_compute, total ? 100.0*icpp_prof_graph_compute/total : 0);
    if (icpp_prof_decode_calls > 0) {
        fprintf(stderr, "ICPP-PROF: per_call: build=%llu alloc=%llu inputs=%llu compute=%llu\n",
                icpp_prof_graph_build/icpp_prof_decode_calls,
                icpp_prof_graph_alloc/icpp_prof_decode_calls,
                icpp_prof_set_inputs/icpp_prof_decode_calls,
                icpp_prof_graph_compute/icpp_prof_decode_calls);
    }
    fflush(stderr);
    // reset for next call
    icpp_prof_graph_build = 0;
    icpp_prof_graph_alloc = 0;
    icpp_prof_set_inputs = 0;
    icpp_prof_graph_compute = 0;
    icpp_prof_decode_calls = 0;
}
