...
// ICPP-PROFILING: per-op instruction counter
#ifdef __wasm__
__attribute__((import_module("ic0")))
__attribute__((import_name("performance_counter")))
uint64_t icpp_perf_counter_op(uint32_t counter_type);
#else
static inline uint64_t icpp_perf_counter_op(uint32_t t) { (void)t; return 0; }
#endif

#define ICPP_OP_PROFILE_MAX 80
static uint64_t icpp_op_instr[ICPP_OP_PROFILE_MAX];
static uint64_t icpp_op_count[ICPP_OP_PROFILE_MAX];

void icpp_op_profile_reset(void) {
    memset(icpp_op_instr, 0, sizeof(icpp_op_instr));
    memset(icpp_op_count, 0, sizeof(icpp_op_count));
}

void icpp_op_profile_dump(void) {
    uint64_t total = 0;
    for (int i = 0; i < ICPP_OP_PROFILE_MAX; i++) total += icpp_op_instr[i];
    fprintf(stderr, "ICPP-PROF: === PER-OP BREAKDOWN (total=%llu) ===\n", total);
    for (int i = 0; i < ICPP_OP_PROFILE_MAX; i++) {
        if (icpp_op_count[i] > 0) {
            fprintf(stderr, "ICPP-PROF: op[%2d] %-24s calls=%4llu instr=%12llu (%5.1f%%) avg=%llu\n",
                    i, ggml_op_name((enum ggml_op)i),
                    icpp_op_count[i], icpp_op_instr[i],
                    total ? 100.0*icpp_op_instr[i]/total : 0,
                    icpp_op_instr[i] / icpp_op_count[i]);
        }
    }
    fflush(stderr);
}
// ICPP-PROFILING: MUL_MAT internal breakdown
static uint64_t icpp_mm_from_float = 0;
static uint64_t icpp_mm_barrier = 0;
static uint64_t icpp_mm_vec_dot = 0;
static uint64_t icpp_mm_other = 0;
static uint64_t icpp_mm_calls = 0;

void icpp_mm_profile_reset(void) {
    icpp_mm_from_float = 0;
    icpp_mm_barrier = 0;
    icpp_mm_vec_dot = 0;
    icpp_mm_other = 0;
    icpp_mm_calls = 0;
...
void icpp_mm_profile_dump(void) {
    uint64_t total = icpp_mm_from_float + icpp_mm_barrier + icpp_mm_vec_dot + icpp_mm_other;
    fprintf(stderr, "ICPP-PROF: === MUL_MAT INTERNAL (%llu calls) ===\n", icpp_mm_calls);
    fprintf(stderr, "ICPP-PROF: mm_from_float=%llu (%.1f%%)\n", icpp_mm_from_float, total ? 100.0*icpp_mm_from_float/total : 0);
    fprintf(stderr, "ICPP-PROF: mm_barrier=%llu (%.1f%%)\n", icpp_mm_barrier, total ? 100.0*icpp_mm_barrier/total : 0);
    fprintf(stderr, "ICPP-PROF: mm_vec_dot=%llu (%.1f%%)\n", icpp_mm_vec_dot, total ? 100.0*icpp_mm_vec_dot/total : 0);
    fprintf(stderr, "ICPP-PROF: mm_other=%llu (%.1f%%)\n", icpp_mm_other, total ? 100.0*icpp_mm_other/total : 0);
    if (icpp_mm_calls > 0) {
        fprintf(stderr, "ICPP-PROF: mm_per_call: from_float=%llu barrier=%llu vec_dot=%llu other=%llu\n",
                icpp_mm_from_float/icpp_mm_calls, icpp_mm_barrier/icpp_mm_calls,
                icpp_mm_vec_dot/icpp_mm_calls, icpp_mm_other/icpp_mm_calls);
    }
    fflush(stderr);
}
// ICPP-PROFILING-END
...
UseGgmlGemm1:;
    ...
    uint64_t _mm_t0 = icpp_perf_counter_op(0);
    ...
    uint64_t _mm_t1 = icpp_perf_counter_op(0);
    icpp_mm_from_float += _mm_t1 - _mm_t0;
    ...
    uint64_t _mm_t2 = icpp_perf_counter_op(0);
    icpp_mm_barrier += _mm_t2 - _mm_t1;
...
UseGgmlGemm2:;
    ...
    uint64_t _mm_t3 = icpp_perf_counter_op(0);
    ...
    uint64_t _mm_t4 = icpp_perf_counter_op(0);
    icpp_mm_vec_dot += _mm_t4 - _mm_t3;
    icpp_mm_other += _mm_t3 - _mm_t2; // chunk setup overhead
    icpp_mm_calls++;
}
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        uint64_t _op_start = icpp_perf_counter_op(0);
        ggml_compute_forward(&params, node);
        uint64_t _op_end = icpp_perf_counter_op(0);
        if (node->op >= 0 && node->op < ICPP_OP_PROFILE_MAX) {
            icpp_op_instr[node->op] += _op_end - _op_start;
            icpp_op_count[node->op]++;
        }
