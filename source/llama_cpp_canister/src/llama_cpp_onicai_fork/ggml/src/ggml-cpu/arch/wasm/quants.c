...
void ggml_vec_dot_q4_0_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
...
#if defined __wasm_simd128__
    v128_t sumv0 = wasm_f32x4_splat(0.0f);
    v128_t sumv1 = wasm_f32x4_splat(0.0f);

    const v128_t m4b = wasm_i8x16_splat(0x0F);
    const v128_t s8b = wasm_i8x16_splat(0x8);

    // Macro: unpack Q4_0 block and dot-product with Q8_0 block → i32x4
    #define Q4_DOT_BLOCK(xb, yb) ({                                          \
        v128_t vx  = wasm_v128_load((xb)->qs);                               \
        v128_t vxl = wasm_i8x16_sub(wasm_v128_and(vx, m4b), s8b);           \
        v128_t vxh = wasm_i8x16_sub(wasm_u8x16_shr(vx, 4),  s8b);          \
        v128_t yl  = wasm_v128_load((yb)->qs);                               \
        v128_t yh  = wasm_v128_load((yb)->qs + 16);                          \
        wasm_i32x4_add(                                                       \
            wasm_i32x4_add(                                                   \
                wasm_i32x4_dot_i16x8(wasm_i16x8_extend_low_i8x16(vxl),      \
                                     wasm_i16x8_extend_low_i8x16(yl)),       \
                wasm_i32x4_dot_i16x8(wasm_i16x8_extend_high_i8x16(vxl),     \
                                     wasm_i16x8_extend_high_i8x16(yl))),     \
            wasm_i32x4_add(                                                   \
                wasm_i32x4_dot_i16x8(wasm_i16x8_extend_low_i8x16(vxh),      \
                                     wasm_i16x8_extend_low_i8x16(yh)),       \
                wasm_i32x4_dot_i16x8(wasm_i16x8_extend_high_i8x16(vxh),     \
                                     wasm_i16x8_extend_high_i8x16(yh))));    \
    })

    // Unrolled ×4 blocks per iteration (2 pairs of 2 blocks)
    // Two independent accumulators (sumv0, sumv1) for ILP
    for (; ib + 3 < nb; ib += 4) {
        v128_t dp0 = Q4_DOT_BLOCK(&x[ib+0], &y[ib+0]);
        v128_t dp1 = Q4_DOT_BLOCK(&x[ib+1], &y[ib+1]);
        v128_t dp2 = Q4_DOT_BLOCK(&x[ib+2], &y[ib+2]);
        v128_t dp3 = Q4_DOT_BLOCK(&x[ib+3], &y[ib+3]);

        float s0 = GGML_CPU_FP16_TO_FP32(x[ib+0].d) * GGML_CPU_FP16_TO_FP32(y[ib+0].d);
        float s1 = GGML_CPU_FP16_TO_FP32(x[ib+1].d) * GGML_CPU_FP16_TO_FP32(y[ib+1].d);
        float s2 = GGML_CPU_FP16_TO_FP32(x[ib+2].d) * GGML_CPU_FP16_TO_FP32(y[ib+2].d);
        float s3 = GGML_CPU_FP16_TO_FP32(x[ib+3].d) * GGML_CPU_FP16_TO_FP32(y[ib+3].d);

        sumv0 = wasm_f32x4_add(sumv0, wasm_f32x4_mul(wasm_f32x4_convert_i32x4(dp0), wasm_f32x4_splat(s0)));
        sumv1 = wasm_f32x4_add(sumv1, wasm_f32x4_mul(wasm_f32x4_convert_i32x4(dp1), wasm_f32x4_splat(s1)));
        sumv0 = wasm_f32x4_add(sumv0, wasm_f32x4_mul(wasm_f32x4_convert_i32x4(dp2), wasm_f32x4_splat(s2)));
        sumv1 = wasm_f32x4_add(sumv1, wasm_f32x4_mul(wasm_f32x4_convert_i32x4(dp3), wasm_f32x4_splat(s3)));
    }

    // Remainder: 2 blocks at a time
    for (; ib + 1 < nb; ib += 2) {
        v128_t dp0 = Q4_DOT_BLOCK(&x[ib+0], &y[ib+0]);
        v128_t dp1 = Q4_DOT_BLOCK(&x[ib+1], &y[ib+1]);
    ...
    sumv0 = wasm_f32x4_add(sumv0, sumv1);
    sumf = wasm_f32x4_extract_lane(sumv0, 0) + wasm_f32x4_extract_lane(sumv0, 1) +
           wasm_f32x4_extract_lane(sumv0, 2) + wasm_f32x4_extract_lane(sumv0, 3);
...
void ggml_vec_dot_q6_K_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    // Vectorized unpack constants (Paper 1.5 C4 — strict SIMD only, deterministic)
    ...
    for (int i = 0; i < nb; ++i) {
        // Unpack 6-bit quantized data into aux8 (vectorized — Paper 1.5 C4)
...
void ggml_vec_dot_tq2_0_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (size_t j = 0; j < sizeof(x->qs); j += 32) {
            const v128_t qx0 = wasm_v128_load(x[i].qs + j);
            const v128_t qx1 = wasm_v128_load(x[i].qs + j + 16);

            const v128_t sqx0 = wasm_v128_and(qx0, m3);
            const v128_t sqx1 = wasm_v128_and(qx1, m3);
            const v128_t sqx2 = wasm_v128_and(wasm_u8x16_shr(qx0, 2), m3);
            const v128_t sqx3 = wasm_v128_and(wasm_u8x16_shr(qx1, 2), m3);
            const v128_t sqx4 = wasm_v128_and(wasm_u8x16_shr(qx0, 4), m3);
            const v128_t sqx5 = wasm_v128_and(wasm_u8x16_shr(qx1, 4), m3);
            const v128_t sqx6 = wasm_u8x16_shr(qx0, 6);
            const v128_t sqx7 = wasm_u8x16_shr(qx1, 6);

            const v128_t qy0 = wasm_v128_load(y[i].qs + j*4 +   0);
            const v128_t qy1 = wasm_v128_load(y[i].qs + j*4 +  16);
            const v128_t qy2 = wasm_v128_load(y[i].qs + j*4 +  32);
            const v128_t qy3 = wasm_v128_load(y[i].qs + j*4 +  48);
            const v128_t qy4 = wasm_v128_load(y[i].qs + j*4 +  64);
            const v128_t qy5 = wasm_v128_load(y[i].qs + j*4 +  80);
            const v128_t qy6 = wasm_v128_load(y[i].qs + j*4 +  96);
            const v128_t qy7 = wasm_v128_load(y[i].qs + j*4 + 112);

#define TQ2_DOT(sx, sy) \
            sumi0 = wasm_i32x4_add(sumi0, wasm_i32x4_dot_i16x8( \
                wasm_i16x8_extend_low_i8x16(sx), wasm_i16x8_extend_low_i8x16(sy))); \
            sumi1 = wasm_i32x4_add(sumi1, wasm_i32x4_dot_i16x8( \
                wasm_i16x8_extend_high_i8x16(sx), wasm_i16x8_extend_high_i8x16(sy)));
...
#undef TQ2_DOT
        ...
        v128_t total_sumi = wasm_i32x4_add(sumi0, sumi1);

        const v128_t ysum0 = wasm_v128_load(y[i].bsums);
        const v128_t ysum1 = wasm_v128_load(y[i].bsums + 8);
        const v128_t ysum = wasm_i16x8_add(ysum0, ysum1);
        total_sumi = wasm_i32x4_sub(total_sumi,
            wasm_i32x4_add(wasm_i32x4_extend_low_i16x8(ysum),
                           wasm_i32x4_extend_high_i16x8(ysum)));

        const float d = GGML_CPU_FP16_TO_FP32(x[i].d) * y[i].d;
        sumf += d * (float)(wasm_i32x4_extract_lane(total_sumi, 0) +
                            wasm_i32x4_extract_lane(total_sumi, 1) +
                            wasm_i32x4_extract_lane(total_sumi, 2) +
                            wasm_i32x4_extract_lane(total_sumi, 3));
    }
