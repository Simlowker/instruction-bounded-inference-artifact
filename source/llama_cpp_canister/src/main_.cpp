...
// ICPP-PROFILING: ICP instruction counter
#ifdef __wasm__
__attribute__((import_module("ic0")))
__attribute__((import_name("performance_counter")))
extern "C" uint64_t icpp_perf_counter(uint32_t counter_type);
#else
static inline uint64_t icpp_perf_counter(uint32_t t) { (void)t; return 0; }
#endif
#define ICPP_PERF() icpp_perf_counter(0)
// defined in llama-context.cpp
extern void icpp_dump_decode_profile();
// defined in ggml-cpu.c
extern "C" void icpp_op_profile_reset();
extern "C" void icpp_op_profile_dump();
extern "C" void icpp_mm_profile_reset();
extern "C" void icpp_mm_profile_dump();
// ICPP-PATCH-END
...
// ICPP-PATCH-START-C3
// Returns the on-disk size of a file (bytes) via std::ifstream::tellg, or -1
// on error. Portable under the icpp-pro WASI polyfill (same mechanism as
// file_is_empty above).
static long long icpp_c3_file_size(const std::string &path) {
...
int main_(int argc, char **argv, std::string principal_id, bool load_model_only,
  ...
  // ICPP-PATCH: call counter for diagnostics
  static int call_count = 0;
  call_count++;
  uint64_t perf_start = ICPP_PERF();
  fprintf(stderr, "ICPP-PROF: call #%d start=%llu\n", call_count, perf_start);
  fflush(stderr);

  // ICPP-PATCH-START-C3
  // Accumulators for prompt-cache IO overhead characterization (Paper 1.5 C3).
  // Emitted once at end-of-main_ AFTER all op-profile dumps so they survive
  // the canister log ring buffer (Task 14).
  uint64_t c3_load_instr = 0;
  uint64_t c3_save_instr_intermediate = 0;
  uint64_t c3_save_instr_terminal = 0;
  long long c3_load_bytes = -1;
  long long c3_save_bytes_intermediate = -1;
  long long c3_save_bytes_terminal = -1;
  size_t c3_load_tokens = 0;
  size_t c3_save_tokens_intermediate = 0;
  size_t c3_save_tokens_terminal = 0;
  int c3_load_called = 0;
  int c3_save_intermediate_called = 0;
  int c3_save_terminal_called = 0;
  int c3_load_ok = -1;
  // ICPP-PATCH-END-C3
  ...
  // logits_all was removed upstream — perplexity check no longer needed here
  ...
  // ICPP-PATCH: backend must stay alive as long as static model exists.
  // llama_backend_free() destroys quantization tables the model references.
  ...
  static llama_model *model = nullptr; // ICPP-PATCH: static to persist across canister calls
  static llama_context *ctx = nullptr; // ICPP-PATCH: static — WASM can't free+realloc (fragmentation)
  common_sampler *smpl = nullptr;

  uint64_t perf_after_parse = ICPP_PERF();
  fprintf(stderr, "ICPP-PROF: after_parse=%llu delta=%llu\n", perf_after_parse, perf_after_parse - perf_start);
  fflush(stderr);
  ...
  // ICPP-PATCH: static to keep ownership across calls
  static common_init_result_ptr llama_init;

  // ICPP-PATCH: Load once, reuse on subsequent calls.
  // WASM has no memory.shrink — reload causes fragmentation → OOM.
  if (model == nullptr) {
    LOG_INF("%s: loading model for the first time\n", __func__);
    uint64_t icpp_load_common_init_pre = ICPP_PERF();
    fprintf(stderr, "ICPP-LOAD: begin common_init_from_params\n");
    fflush(stderr);
    llama_init = common_init_from_params(params);
    uint64_t icpp_load_common_init_post = ICPP_PERF();
    fprintf(stderr,
            "ICPP-LOAD: end common_init_from_params delta=%llu\n",
            (unsigned long long)(icpp_load_common_init_post -
                                 icpp_load_common_init_pre));
    fflush(stderr);

    if (!llama_init) {
      LOG_ERR("%s: error: common_init_from_params returned null\n", __func__);
      icpp_error_msg = std::string(__func__) + ": error: common_init_from_params returned null";
      return 1;
    }
    ...
    if (model == NULL) {
      ...
      icpp_error_msg = std::string(__func__) + ": error: unable to load model)";
      return 1;
    }
  } else {
    LOG_INF("%s: reusing loaded model + context\n", __func__);
  }

  if (load_model_only) {
    uint64_t perf_load_only_end = ICPP_PERF();
    fprintf(stderr,
            "ICPP-LOAD: load_model_only_total=%llu\n",
            (unsigned long long)(perf_load_only_end - perf_start));
    fflush(stderr);
    return 0;
  }
  ...
  uint64_t perf_after_model = ICPP_PERF();
  fprintf(stderr, "ICPP-PROF: after_model=%llu delta_from_start=%llu\n", perf_after_model, perf_after_model - perf_start);
  fflush(stderr);
  ...
  const bool has_chat_template =
      common_chat_templates_was_explicit(chat_templates.get()) && chat_templates;
...
#else
  // No chat templates — always disable conversation mode
  params.conversation_mode = COMMON_CONVERSATION_MODE_DISABLED;
#endif
  ...
  if (!path_session.empty()) {
    ...
    if (!file_exists(path_session)) {
      ...
      // ICPP-PATCH-START-C3
      uint64_t icpp_c3_load_pre = ICPP_PERF();
      c3_load_bytes = icpp_c3_file_size(path_session);
      bool icpp_c3_load_ok = llama_state_load_file(
              ctx, path_session.c_str(), session_tokens.data(),
              session_tokens.capacity(), &n_token_count_out);
      uint64_t icpp_c3_load_post = ICPP_PERF();
      c3_load_instr = icpp_c3_load_post - icpp_c3_load_pre;
      c3_load_tokens = n_token_count_out;
      c3_load_ok = icpp_c3_load_ok ? 1 : 0;
      c3_load_called = 1;
      fprintf(stderr,
              "C3-IO: state_load instr=%llu ok=%d tokens=%zu bytes=%lld path=%s\n",
              (unsigned long long)c3_load_instr,
              c3_load_ok,
              c3_load_tokens,
              c3_load_bytes,
              path_session.c_str());
      ...
      // ICPP-PATCH-END-C3
        ...
        // ICPP-PATCH-START
        icpp_error_msg =
            std::string(__func__) + ": error: failed to load session file '" + path_session + "')";
        // ICPP-PATCH-END
        return 1;
      }
  ...
  auto chat_add_and_format = [&chat_msgs,
    ...
    auto formatted =
        common_chat_format_single(chat_templates.get(), chat_msgs,
  ...
  if (!embd_inp_is_session_tokens) {
    ...
    if ((int)embd_inp.size() > n_ctx - 4) {
      ...
      // ICPP-PATCH-START
      icpp_error_msg =
          std::string(__func__) + ": error: prompt is too long (" + std::to_string((int)embd_inp.size()) + " tokens, max " + std::to_string(n_ctx - 4) + ")";
      // ICPP-PATCH-END
      return 1;
    }
    // ICPP-PATCH-START
  ...
  if (llama_model_has_encoder(model)) {
    ...
    if (llama_encode(ctx, llama_batch_get_one(enc_input_buf, enc_input_size))) {
      ...
      // ICPP-PATCH-START
      icpp_error_msg = std::string(__func__) + ": error: failed to eval (-1-)";
      // ICPP-PATCH-END
      return 1;
    }
  ...
  icpp_op_profile_reset();
  icpp_mm_profile_reset();
  uint64_t perf_before_loop = ICPP_PERF();
  fprintf(stderr, "ICPP-PROF: before_loop=%llu delta_from_start=%llu\n", perf_before_loop, perf_before_loop - perf_start);
  fflush(stderr);
  int prof_tokens_generated = 0;
  uint64_t prof_decode_total = 0;
  uint64_t prof_sample_total = 0;
  ...
  while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
    ...
    if (!embd.empty()) {
      ...
      if (ga_n == 1) {
        ...
        if (n_past + (int)embd.size() >= n_ctx) {
          ...
          llama_memory_seq_rm(llama_get_memory(ctx), 0, params.n_keep,
                                params.n_keep + n_discard);
          llama_memory_seq_add(llama_get_memory(ctx), 0, params.n_keep + n_discard, n_past,
                                 -n_discard);
        ...
        while (n_past >= ga_i + ga_w) {
          ...
          llama_memory_seq_div(llama_get_memory(ctx), 0, ga_i + ib * bd, ga_i + ib * bd + ga_w,
                                 ga_n);
          llama_memory_seq_add(llama_get_memory(ctx), 0, ga_i + ib * bd + ga_w,
                                 n_past + ib * bd, dd);
      ...
      for (int i = 0; i < (int)embd.size(); i += params.n_batch) {
        ...
        uint64_t perf_pre_decode = ICPP_PERF();
        if (llama_decode(ctx, llama_batch_get_one(&embd[i], n_eval))) {
          LOG_ERR("%s : failed to eval\n", __func__);
          // ICPP-PATCH-START
          icpp_error_msg =
              std::string(__func__) + ": error: failed to eval (-3-)";
          // ICPP-PATCH-END
          return 1;
        }

        prof_decode_total += ICPP_PERF() - perf_pre_decode;
        n_past += n_eval;
    ...
    if ((int)embd_inp.size() <= n_consumed && !is_interacting) {
      ...
      if (!path_session.empty() && need_to_save_session &&
        ...
        // ICPP-PATCH-START-C3
        uint64_t icpp_c3_save_pre = ICPP_PERF();
        llama_state_save_file(ctx, path_session.c_str(), session_tokens.data(),
                              session_tokens.size());
        uint64_t icpp_c3_save_post = ICPP_PERF();
        c3_save_instr_intermediate = icpp_c3_save_post - icpp_c3_save_pre;
        c3_save_bytes_intermediate = icpp_c3_file_size(path_session);
        c3_save_tokens_intermediate = session_tokens.size();
        c3_save_intermediate_called = 1;
        fprintf(stderr,
                "C3-IO: state_save instr=%llu tokens=%zu bytes=%lld site=intermediate path=%s\n",
                (unsigned long long)c3_save_instr_intermediate,
                c3_save_tokens_intermediate,
                c3_save_bytes_intermediate,
                path_session.c_str());
        fflush(stderr);
        // ICPP-PATCH-END-C3
      ...
      uint64_t perf_pre_sample = ICPP_PERF();
      const llama_token id = common_sampler_sample(smpl, ctx, -1);

      common_sampler_accept(smpl, id, /* accept_grammar= */ true);
      prof_sample_total += ICPP_PERF() - perf_pre_sample;
      prof_tokens_generated++;
  ...
  if (!path_session.empty() && params.prompt_cache_all &&
    ...
    llama_state_save_file(ctx, path_session.c_str(), session_tokens.data(),
                          session_tokens.size());
    uint64_t icpp_c3_save_post = ICPP_PERF();
    c3_save_instr_terminal = icpp_c3_save_post - icpp_c3_save_pre;
    c3_save_bytes_terminal = icpp_c3_file_size(path_session);
    c3_save_tokens_terminal = session_tokens.size();
    c3_save_terminal_called = 1;
    fprintf(stderr,
            "C3-IO: state_save instr=%llu tokens=%zu bytes=%lld site=terminal path=%s\n",
            (unsigned long long)c3_save_instr_terminal,
            c3_save_tokens_terminal,
            c3_save_bytes_terminal,
            path_session.c_str());
    fflush(stderr);
    // ICPP-PATCH-END-C3
  }
  ...
  // ICPP-PROFILING summary
  uint64_t perf_end = ICPP_PERF();
  uint64_t total = perf_end - perf_start;
  uint64_t overhead = total - prof_decode_total - prof_sample_total;
  fprintf(stderr, "ICPP-PROF: === SUMMARY call #%d ===\n", call_count);
  fprintf(stderr, "ICPP-PROF: total_instructions=%llu\n", total);
  fprintf(stderr, "ICPP-PROF: decode_total=%llu (%.1f%%)\n", prof_decode_total, 100.0*prof_decode_total/total);
  fprintf(stderr, "ICPP-PROF: sample_total=%llu (%.1f%%)\n", prof_sample_total, 100.0*prof_sample_total/total);
  fprintf(stderr, "ICPP-PROF: overhead=%llu (%.1f%%)\n", overhead, 100.0*overhead/total);
  fprintf(stderr, "ICPP-PROF: tokens_generated=%d\n", prof_tokens_generated);
  if (prof_tokens_generated > 0) {
    fprintf(stderr, "ICPP-PROF: instructions_per_token=%llu (decode only: %llu)\n",
            total / prof_tokens_generated, prof_decode_total / prof_tokens_generated);
  }
  fprintf(stderr, "ICPP-PROF: setup_cost(start→loop)=%llu\n", perf_before_loop - perf_start);
  fflush(stderr);
  icpp_dump_decode_profile();
  icpp_op_profile_dump();
  icpp_mm_profile_dump();
  ...
  // ICPP-PATCH: do NOT call llama_backend_free() here — the static model
  // is still alive and references the quantization tables it initialized.
  // Backend is freed in icpp_free_model() instead.

  ggml_threadpool_free_fn(threadpool);
  ggml_threadpool_free_fn(threadpool_batch);

  // ICPP-PATCH-START-C3
  // Emit a single-line summary AFTER the op profile dumps so the canister
  // log ring buffer retains it (Task 14 — C3 IO overhead characterization).
  fprintf(stderr,
          "C3-IO-END: call=%d "
          "load_called=%d load_ok=%d load_instr=%llu load_tokens=%zu load_bytes=%lld "
          "save_intermediate_called=%d save_intermediate_instr=%llu save_intermediate_tokens=%zu save_intermediate_bytes=%lld "
          "save_terminal_called=%d save_terminal_instr=%llu save_terminal_tokens=%zu save_terminal_bytes=%lld\n",
          call_count,
          c3_load_called, c3_load_ok,
          (unsigned long long)c3_load_instr, c3_load_tokens, c3_load_bytes,
          c3_save_intermediate_called,
          (unsigned long long)c3_save_instr_intermediate,
          c3_save_tokens_intermediate, c3_save_bytes_intermediate,
          c3_save_terminal_called,
          (unsigned long long)c3_save_instr_terminal,
          c3_save_tokens_terminal, c3_save_bytes_terminal);
  fflush(stderr);
  // ICPP-PATCH-END-C3
