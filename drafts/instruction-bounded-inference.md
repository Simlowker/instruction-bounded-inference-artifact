...
**(R3) Floor evidence.** A purpose-built TQ2_0 (2-bit ternary in the GGUF binary format used by llama.cpp [10]) WASM SIMD path on purpose-trained TriLM models decodes at the empirical floor of the measured decode path: `α_eff = 1.03` at 560M, with a coarse integer-token consistency check at 3.9B (`N_MAX = 5`, asymmetric −17%/+0.2% resolution, implying `α_eff ≤ 1.002`) — and byte-identical outputs across a local replica, 13-replica Swiss Subnet consensus, and a standard ICP mainnet subnet. Here `α_eff = 1` is one metered unit per FLOP at the `2P` count — the observed floor of single-token decode on this stack, not a cost-table minimum: batched encode and isolated ternary kernel benches measure below it (§2, §5.3, §7.1). The evidence spans one model family and one format; we do not claim closed saturation.
...
- **Purpose-built TQ2_0 ternary path**: α_eff ≈ 1.00 (empirical decode-path floor)

The floor is load-bearing, so we state precisely what it is — and what it is not. Setting α_eff = 1 corresponds to one metered unit per FLOP at the `2P` count (equivalently, two units per multiply-accumulate, since `2P` counts multiply and add separately). It is _not_ a cost-table minimum: fused SIMD instructions retire several MACs at once, and both the isolated ternary kernel bench (`Ternary_DOT_i32x4dot_x4`, α_kernel = 0.817, `artifact/data/kernel/matmul_bench.csv`) and the batched encoder path (α_embed ≈ 0.53, §7.1) measure below 1 on the same cost model. What makes 1 the operative floor for _decode_ is structural: single-token matrix-vector decode streams every weight through load + dequantize + MAC exactly once per token, so the per-weight load/dequant instructions cannot be amortized — unlike batched encode, where they are shared across the token batch, and unlike matrix-matrix kernel benches, where operand reuse hides them. On the currently-integrated decode kernels, that per-weight overhead lands the end-to-end coefficient at α_eff ≈ 1, and no end-to-end decode deployment on this stack is observed below it. The claim "the TQ2_0 path reaches the floor" (§5) should therefore be read as measurement evidence that no large remaining margin is visible on that measured decode path at integer-token resolution — an _empirical_ floor of the current path, not a bound future kernels cannot cross: the ternary-native DOT ×4 microkernel already measures α_kernel = 0.817 on isolated benches (§5.3), and integrating it at model level is the obvious next step.
...
Legacy GPT-2 rows land at α ≈ 2.0 on nominal parameter counts — systematically above the modern cluster, and only on ICP. Two effects compose here. First, a parameter-convention effect: GPT-2's audited GGUF counts exceed nominal by 24–32% (untied vocabulary and position embeddings; `discrepancies.csv`), and on audited counts the same rows land at α ≈ 1.39–1.46, at or below the modern cluster. Second, a residual ICP-specific software component: on native Apple M4 Max the same models show no regime split (`artifact/data/native/m4_max_baseline.csv`), so what remains after the convention correction is attributable to the llama.cpp GPT-2 code path on this stack, not to the architecture itself. The α ≈ 2.0 headline is thus partly a nominal-convention artifact; we retain GPT-2 as a sensitivity regime, excluded from the modern fit either way.
...
**Sensitivity to the parameter convention.** `2P` uses nominal (published) parameter totals in our fits, applied uniformly. GGUF-audited tensor counts are _file-dependent_ and occasionally exceed nominal by 15–32% (e.g. the Qwen 2.5 0.5B Q8_0-instruct export audits at 630 M vs 494 M nominal, while its Q4_0/TQ2_0 exports audit at 494 M; GPT-2 audits +24–32%; `CLAIMS-EVIDENCE-MATRIX.md` discrepancies log). Recalibrating on audited counts therefore shifts α_eff _non-uniformly_ — affected models move down by their discrepancy (Qwen Q8_0: 1.33 → ≈ 1.06) while others barely move — preserving the cluster's ordering and the α ≈ 1 floor but not every individual value. We report the nominal convention throughout and flag rows with a large audit gap in the registry.
