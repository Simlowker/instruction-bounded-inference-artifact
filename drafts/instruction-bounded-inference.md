...
| `f32.mul` vs `i32.mul` | Identical           | 2× per opcode table; ~1.05× at SIMD-kernel level (§4.2) |
...
with `IO_instr(n, c) = save_instr(n, c) + load_instr(n, c) ≈ (a_s + b_s · n) + (a_l + b_l · n)` fitted from instrumented canister runs (an instrumented build — ggml commit `bb238b9` — logs `save_instr` and `load_instr` per call; for the Qwen fit, `b_l ≈ 0`: the load cost resolves to a near-constant term). Units: `B` and `IO_instr` in ICP instruction-cost units; `α_eff × 2P` in instructions per token.
